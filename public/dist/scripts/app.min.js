...
"use strict";function showPopup(title,message,sh,showButton){if(401!=message.status){var popupModal=sh.openModal("globalPopup.html",title,message,showButton);return popupModal.result.then(function(result){return result})}}var copenhagenApp=angular.module("copenhagenApp",["ngAnimate","ui.router","ui.bootstrap","mwl.calendar","angularFileUpload","ui.toggle","moment-picker","countrySelect","rzModule","bootstrapLightbox","ngCookies","textAngular","ngSanitize"]).config(["$httpProvider","$qProvider",function($httpProvider,$qProvider){$httpProvider.defaults.stripTrailingSlashes=!1,$httpProvider.defaults.headers.common["X-CSRF-TOKEN"]=CopenhagenAppConfig.csrfToken,$qProvider.errorOnUnhandledRejections(!1),$httpProvider.interceptors.push("authHttpResponseInterceptor")}]).run(["$rootScope","$window","API","$location",function($rootScope,$window,API,$location){$rootScope.currentUser=API.getCurrentUser(),$rootScope.booking=API.getBookingData(),$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){toState.external&&(event.preventDefault(),$window.open(toState.url,"_self"))}),$rootScope.$on("$stateChangeSuccess",function(event,toState,toParams,fromState,fromParams){$rootScope.title=toState.title||"Long Term Stay Apartments | Cebu Serviced Apartments",$rootScope.description=toState.description||"Copenhagen, a long term stay apartments with hotel accommodation. It&#039;s Cebu serviced apartments &amp; corporate housing for long term stay serviced apartments.",$rootScope.keywords=toState.keywords||"copenhagen,bookings,rooms",$rootScope.canonical=$location.absUrl()})}]).filter("range",function(){return function(input,min,max){min=parseInt(min),max=parseInt(max);for(var i=min;i<max;i++)input.push(i);return input}}).factory("sh",function($uibModal){function openModal(_templateUrl,_title,_message,_showButton,_resolve){if(void 0==_resolve)var _resolve={modalTitle:function(){return _title},bodyMessage:function(){return _message}};return $uibModal.open({animation:!0,templateUrl:_templateUrl,backdrop:"static",keyboard:!1,controller:function($scope,$uibModalInstance,modalTitle,bodyMessage){$scope.modalTitle=modalTitle,$scope.modalMessage=bodyMessage,$scope.showButton=_showButton,$scope.ok=function(){$uibModalInstance.close("ok")},$scope.cancel=function(){$uibModalInstance.dismiss("cancel")}},resolve:_resolve})}return{openModal:openModal}}).filter("unsafe",["$sce",function($sce){return function(text){return $sce.trustAsHtml(text)}}]).filter("trim",function(){return function(value){return angular.isString(value)?value.replace(/^\s+|\s+$/g,""):value}}).directive("pagenotfound",function(){var directive={};return directive.restrict="E",directive.template='<div class="text-center box-404"><h1>404</h1><p>Sorry the page you are looking for is not found.</p><a ui-sref="home">Home Page</a></div>',directive.scope={},directive.compile=function(element,attributes){var linkFunction=function($scope,element,attributes){};return linkFunction},directive});
...
copenhagenApp.factory("API",["$http","$rootScope","$state","$cookies",function($http,$rootScope,$state,$cookies){var urlBase="",api={};return api.login=function(auth){return $http.post(urlBase+"/login",auth)},api.logout=function(){return $cookies.remove("currentUser"),$http.get(urlBase+"/logout")},api.isAuthenticated=function(){return void 0!=$cookies.get("currentUser")&&$cookies.get("currentUser")},api.setCurrentUser=function(user){$cookies.put("currentUser",JSON.stringify(user[0]))},api.getCurrentUser=function(){var _user=$cookies.get("currentUser");if(null!=_user)return JSON.parse(_user)},api.updateProfile=function(id,user){return $http.put(urlBase+"/profile/"+id,user)},api.seoMeta=function(type,id){return $http.get(urlBase+"/seo/"+type+"/"+id)},api.saveSeoMeta=function(seo){return $http.put(urlBase+"/seo/"+seo.id,seo)},api.getRates=function(){return $http.get(urlBase+"/rates")},api.saveRate=function(rate,id){return id?$http.put(urlBase+"/rates/"+id,rate):$http.post(urlBase+"/rates",rate)},api.deleteRate=function(id){return $http.delete(urlBase+"/rates/"+id)},api.saveRoom=function(room){return room.id?$http.put(urlBase+"/rooms/"+room.id,room):$http.post(urlBase+"/rooms",room)},api.getRooms=function(){return $http.get(urlBase+"/rooms")},api.getRoomsLists=function(){return $http.get(urlBase+"/rooms/lists")},api.getAvailableRooms=function(){return $http.get(urlBase+"/rooms/available")},api.getRoomBySlug=function(slug){return $http.get(urlBase+"/rooms/"+slug)},api.getRoomById=function(id){return $http.get(urlBase+"/room/"+id)},api.deleteRoom=function(roomID){return $http.delete(urlBase+"/rooms/"+roomID)},api.deletePhoto=function(roomID,photoID){return $http.delete(urlBase+"/rooms/"+roomID+"/photos/"+photoID)},api.getAminities=function(){return $http.get(urlBase+"/rooms/aminities")},api.saveFacility=function(facility){return $http.post(urlBase+"/rooms/aminities",facility)},api.deleteAminities=function(id){return $http.delete(urlBase+"/rooms/aminities/"+id)},api.saveRoomAminities=function(roomID,data){return $http.post(urlBase+"/rooms/"+roomID+"/aminities",data)},api.saveRoomBed=function(roomID,data){return $http.post(urlBase+"/rooms/"+roomID+"/beds",data)},api.deleteRoomBed=function(roomID,bedID){return $http.delete(urlBase+"/rooms/"+roomID+"/beds/"+bedID)},api.getRoomTypes=function(){return $http.get(urlBase+"/rooms/types")},api.saveCalendar=function(calendar){return $http.post(urlBase+"/calendar",calendar)},api.fetchCalendarByRoomIdAndDate=function(roomID,start,end){return $http.get(urlBase+"/rooms/"+roomID+"/calendar/"+start+"/"+end)},api.setBookingData=function(data){$rootScope.booking=data,$cookies.put("Booking",JSON.stringify(data))},api.getBookingData=function(){var _b=$cookies.get("Booking");if(null!=_b)return JSON.parse(_b)},api.book=function(data){return $http.post(urlBase+"/book",data)},api.getBookings=function(){return $http.get(urlBase+"/bookings")},api.sendEmailNotification=function(ref){return $http.post(urlBase+"/booking/notify",{Ref:ref})},api.checkRoomAvailability=function(params){return $http.post(urlBase+"/rooms/availability",params)},api.fetchUnavailableCalendarByRoomId=function(roomID,start,end){return $http.get(urlBase+"/rooms/"+roomID+"/calendar/unavailable/"+start+"/"+end)},api.bookingStep=function(params,step){return $http.post(urlBase+"/booking/step/"+step,params)},api.sendContact=function(params){return $http.post(urlBase+"/contact",params)},api.getDisabledDates=function(){return $http.get(urlBase+"/disable-date")},api.saveDisableDate=function(params){return $http.post(urlBase+"/disable-date",params)},api.deleteDisabledDate=function(id){return $http.delete(urlBase+"/disable-date/"+id)},api.getPages=function(){return $http.get(urlBase+"/pages")},api.getPage=function(id){return $http.get(urlBase+"/pages/"+id)},api.savePage=function(rate,id){return id?$http.put(urlBase+"/pages/"+id,rate):$http.post(urlBase+"/pages",rate)},api.deletePage=function(id){return $http.delete(urlBase+"/pages/"+id)},api.getPageBySlug=function(slug){return $http.get(urlBase+"/page/"+slug)},api}]).factory("authHttpResponseInterceptor",["$q","$location","$cookies",function($q,$location,$cookies){return{response:function(response){return"401"==response.status&&($cookies.remove("currentUser"),window.location="/login",$location.path("/login").search("returnTo",$location.path())),response||$q.when(response)},responseError:function(rejection){return"401"==rejection.status&&($cookies.remove("currentUser"),$location.path("/login").search("returnTo",$location.path())),$q.reject(rejection)}}}]);
